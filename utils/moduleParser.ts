...
export interface ClubType {
  ...
  type_name: string;
  description: string;
  modules: Record<string, ModuleFeature>;
}
...
// Fallback data in case file reading fails or for client-side usage if needed
const FALLBACK_DATA: MasterSchema = {
  "nexlyn_master_schema": {
    "version": "1.0.0",
...
export const getModulesByClubType = (clubType: string): ModuleFeature[] => {
  if (!clubType) return [];

  let data: MasterSchema = FALLBACK_DATA;

  try {
    const filePath = path.join(process.cwd(), 'MODÜL.md');
  ...
  try {
    const normalizedType = clubType.toLowerCase().trim();

    const club = data.nexlyn_master_schema.club_types.find(
      (c) => c.type_id.toLowerCase() === normalizedType || c.type_name.toLowerCase().includes(normalizedType)
    );
    ...
    return Object.values(club.modules);
  } catch {
    return [];
  }
};
